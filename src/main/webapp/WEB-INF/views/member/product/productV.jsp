...
<body>
	...
	<div>
		...
		<div id="productDiv">
			...
			<form action="productVA" method="post">
				<input type="hidden" id="productPrice" name="price" value="${dto.product_price }" >
				<input type="hidden" id="productName" name="name" value="${dto.product_name }" >
				<input type="hidden" id="product_id" name="product_id" value="${dto.product_id }" >
				<input type="hidden" id="productImg" name="img" value="${dto.product_main_img_path }" >
				<div>
					<p><span id="productTitle">${dto.product_name }</span><br><br><span id="price">${dto.product_price }원</span></p>
					<hr>
					<p>제조&nbsp;&nbsp;&nbsp;자체제작<br>
					   원산지&nbsp;&nbsp;&nbsp;국내<br>
					   용량&nbsp;&nbsp;&nbsp;75ml
					...
					<p>
						<label>수량</label>&nbsp;&nbsp;&nbsp;
						<input id="productCount" name="amount" type='number' value="1" min='1' max='999' step='1' onclick="upatePrice();">
					</p>
					<hr>
					<p id="totalPrice">
						<span>TOTAL: </span>
						<span class="price">${dto.product_price }</span>원
					</p>
					<button type="button" onclick="window.open('order','','width=1200, height=800');">구매하기</button>
		...
		<div class="review">
			<div>
				<span>별점:</span>
				<c:if test="${reviewList.review_board_score == 1 }"><img src="images/star1.png" width="80px"></c:if>
				<c:if test="${reviewList.review_board_score == 2 }"><img src="images/star2.png" width="80px"></c:if>
				<c:if test="${reviewList.review_board_score == 3 }"><img src="images/star3.png" width="80px"></c:if>
				<c:if test="${reviewList.review_board_score == 4 }"><img src="images/star4.png" width="80px"></c:if>
				<c:if test="${reviewList.review_board_score == 5 }"><img src="images/star5.png" width="80px"></c:if>
			</div>
			<div>
				<span>아이디: ${reviewList.member_id }</span>&nbsp;&nbsp;&nbsp;
				<fmt:formatDate value="${ reviewList.review_board_date }" pattern="yyyy-MM-dd" var="date" />
				<span>작성일: ${date }</span>&nbsp;&nbsp;&nbsp;
				<span>조회수: ${reviewList.review_board_hit }</span>
			</div>
		</div>
		<hr class="reveiwHr">
		<div class="reviewContent">
			<a href=""><span>${reviewList.review_board_content }</span></a><br>
			<img src="${reviewList.review_board_img_path }" width="130px" onError="this.style.visibility='hidden'">
		</div>
		<hr class="reveiwHr">
		</c:forEach>
		<hr class="mainHr">
		...
		<table>                    
			...
			<c:if test="${dto.member_id eq 'admin' }">
				...
				<c:if test="${dto.member_id != 'admin' }">
					<td>${dto.qna_board_title }<c:if test="${dto.qna_board_lock eq '1' }"><img id="lock" src="images/lock.png" alt="잠금" /></c:if></td>
				</c:if>
				<c:if test="${dto.member_id eq 'admin' }">
				...
				<td>${dto.member_id }</td>
				<fmt:formatDate value="${ dto.qna_board_date }" pattern="yyyy-MM-dd" var="date" />
				<td>${date }</td>
				<td>${ dto.qna_board_hit }</td>
			</tr>
			</c:forEach>
	</table>
	</div>
	...
	<script>
		function upatePrice(){
			...
			price.innerText = (productCount.value * productPrice.value).toLocaleString();
		}
	</script>
</body>
</html>